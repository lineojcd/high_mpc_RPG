...
def main():
    ...
    so_path = os.path.join(
        "high_mpc", "mpc", "saved", "high_mpc.so"
    )  # saved high mpc model (casadi code generation)
    #
    ...
    if args.option == 0:  # collect data
        save_dir = U.get_dir(os.path.join(args.save_dir, "Dataset"))
        save_dir = os.path.join(
            save_dir, datetime.datetime.now().strftime("deep_highmpc-%m-%d-%H-%M-%S")
        )

        #
        logger.configure(dir=save_dir)
        logger.log(
            "***********************Log & Store Hyper-parameters***********************"
        )
        ...
        #
        deep_high_policy.data_collection(
            env=env, logger=logger, save_dir=save_dir, **training_params
        )
    elif args.option == 1:  # train the policy
        data_dir = os.path.join(
            args.save_dir, "Dataset", "deep_highmpc-05-29-22-43-21", "dataset"
        )
