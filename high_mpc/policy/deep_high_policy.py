...
class Actor(Model):
    ...
    def save_weights(self, save_dir, iter):
        save_dir = save_dir + os.path.sep + "act_net"
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        weights_path = save_dir + os.path.sep + "weights_{0}.h5".format(iter)
    ...
    def train_batch(self, obs_batch, act_batch):
        ...
        self._optimizer.apply_gradients(
            zip(act_grad, self._act_net.trainable_variables)
        )
...
class Dataset(object):
    ...
    def save(self, save_dir, n_iter):
        save_dir = save_dir + os.path.sep + "dataset"
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        data_path = save_dir + os.path.sep + "data_{0}".format(n_iter)
    ...
    def load(self, data_dir):
        ...
        for i, data_file in enumerate(glob.glob(os.path.join(data_dir, "*.npz"))):
            size = int(data_file.split(os.path.sep)[-1].split("_")[-1].split(".")[0])
...
def train(
    ...
    for epoch in range(train_epoch):
        #
        train_loss = []
        dataset = train_tf_dataset.shuffle(
