...
class SimVisual(object):
    ...
    def init_animate(
        ...
        return (
            ...
            self.l_pend_az,
            self.l_quad_pos,
            self.l_quad_pred_traj,
            self.l_pend,
            self.l_pend_pred_traj,
            self.l_quad_x,
            self.l_quad_y,
            self.l_quad_z,
            self.l_pend_edge1,
    ...
    def update(self, data_info):
        ...
        if len(self.ts) == 0:
            ...
            if quad_pos_arr[-1, 0] <= 2.0:
                ...
                self.l_pend_pred_traj.set_data(
                    pred_pend_traj[opt_idx, 0], pred_pend_traj[opt_idx, 1]
                )
                # self.l_pend_pred_traj.set_3d_properties(pred_pend_traj[opt_idx, 2])

            #
            self.l_pend.set_data(
                [self.pivot_point[0], pend_obs[kPosX]],
                [self.pivot_point[1], pend_obs[kPosY]],
            )
            ...
            for _, quad_motor in enumerate(self.quad_hist):
                ...
                self.l_quad_x.set_data(
                    [quad_pos_arr[-1, 0], axes_x[0]], [quad_pos_arr[-1, 1], axes_x[1]]
                )
                self.l_quad_x.set_3d_properties([quad_pos_arr[-1, 2], axes_x[2]])
                self.l_quad_y.set_data(
                    [quad_pos_arr[-1, 0], axes_y[0]], [quad_pos_arr[-1, 1], axes_y[1]]
                )
                self.l_quad_y.set_3d_properties([quad_pos_arr[-1, 2], axes_y[2]])
                self.l_quad_z.set_data(
                    [quad_pos_arr[-1, 0], axes_z[0]], [quad_pos_arr[-1, 1], axes_z[1]]
                )
