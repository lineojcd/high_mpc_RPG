...
class Pendulum_v0(object):
    ...
    def _f(self, state):
        ...
        return np.array(
            [
                dot_theta,
                -(
                    (self._gz / self.length) * np.sin(theta)
                    + (self._damping / self._mass) * dot_theta
                ),
            ]
        )
    ...
    def get_corners(
        ...
        y1, z1 = self._to_planar_coordinates(
            self.pivot_point, self.length1, theta + self.delta_theta1
        )
        y2, z2 = self._to_planar_coordinates(
            self.pivot_point, self.length2, theta + self.delta_theta2
        )
        y3, z3 = self._to_planar_coordinates(
            self.pivot_point, self.length3, theta + self.delta_theta3
        )
        y4, z4 = self._to_planar_coordinates(
    ...
    def _f(self, state):
        ...
        return np.array(
            [
                dot_theta,
                -(
                    (self._gz / self._length) * np.sin(theta)
                    + (self._damping / self._mass) * dot_theta
                ),
            ]
        )
