...
class DynamicGap(object):
    def __init__(self, mpc, plan_T, plan_dt):
        ...
        self.planner = Pendulum_v1(
            pivot_point=self.pivot_point, sigma=10, T=self.plan_T, dt=self.plan_dt
        )
        ...
        self.observation_space = Space(
            ...
            high=np.array(
                [10.0, 10.0, 10.0, 2 * np.pi, 2 * np.pi, 2 * np.pi, 10.0, 10.0, 10.0]
            ),
        )

        self.action_space = Space(low=np.array([0.0]), high=np.array([2 * self.plan_T]))
    ...
    def episode(self, u):
        ...
        #
        loss = np.linalg.norm(
            pred_traj[opt_min:opt_max, kPosX : kPosZ + 1]
            - pred_pend_traj_cart[opt_min:opt_max, kPosX : kPosZ + 1]
        )
        rew = -loss
        #
        return rew, opt_node
